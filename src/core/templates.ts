...
export const templates: TrackerTemplate[] = [
  {
    ...
    fields: [
      { name: 'When', type: 'time', config: { includeDate: true, format: '12h', autoNow: true } },
      { name: 'Items', type: 'list', config: { layout: 'pills' } },
      { name: 'Photos', type: 'picture' },
      { name: 'Notes', type: 'longtext', config: { placeholder: 'How was it?', rows: 2 } },
    ],
  },
  {
    id: 'poop',
    ...
    fields: [
      { name: 'Time', type: 'time', config: { includeDate: true, format: '12h', autoNow: true } },
      { name: 'Duration', type: 'duration' },
      { name: 'Description', type: 'longtext', config: { placeholder: 'Brown, solid…', rows: 2 } },
    ],
  },
  {
    id: 'wishlist',
  ...
  },
  {
    ...
    fields: [
      { name: 'Time', type: 'time', config: { includeDate: true, format: '12h', autoNow: true } },
      { name: 'Meal', type: 'text' },
      { name: 'Calories', type: 'number', config: { suffix: 'kcal', decimals: 0 } },
    ],
  },
  {
    id: 'grocery',
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'Mood (1–5)', type: 'number', config: { decimals: 0, min: 1, max: 5 } },
    ],
  },
];
...
export async function createFromTemplate(templateId: string): Promise<string> {
  ...
  const tracker = await createTracker({ name: tpl.name, icon: tpl.icon, color: tpl.color });
  await Promise.all(
    tpl.fields.map((f, i) =>
      addField({
